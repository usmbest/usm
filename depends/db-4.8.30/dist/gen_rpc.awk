...
/^[	 ]*END/ {
	...
	for (i = 0; i < nvars; ++i) {
		if (rpc_type[i] == "ID") {
			...
			printf("\tct_entry *%s_usm;\n", args[i]) >> PFILE
		}
	}
	printf("\n") >> PFILE
	for (i = 0; i < nvars; ++i) {
		if (rpc_type[i] == "ID") {
			printf("\tACTIVATE_CTP(%s_usm, %scl_id, %s);\n", \
			    args[i], args[i], usm_type[i]) >> PFILE
			printf("\t%s = (%s)%s_usm->ct_anyp;\n", \
